			...
			<div class="row mt">
				<div class="col-lg-12">
					<div class="form-panel">
						...
						<form class="form-horizontal style-form" method="post" action="<c:url value='/detailBoard/${ boardDetailMap.board_no}/update'/>">
							...
							<c:if test="${ authorNO eq currentUserNO }">
								<a href="<c:url value='/deleteBoard/${ boardDetailMap.board_no}'/>" class="btn btn-default btn-lg btn-block" >삭제</a>
								<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
								<button type="submit" class="btn btn-default btn-lg btn-block">수정</button>
							</c:if>
							<a href="<c:url value='/boardList'/>" class="btn btn-primary btn-lg btn-block">목록</a>
						</form>
					</div>
				</div>
				<!-- col-lg-12-->
